...
/**
 * Created by zhanpeng on 2017/5/18.
 */
...
public class FFT {
    ...
    public void complexLization(ComplexNum[] complices, short[] pcmData){
        for(int i = 0;i<complices.length;i++){
            complices[i].real = pcmData[i];
            complices[i].imag = 0;
        }
    }
    public ComplexNum[] complexLization(float data[])
    {
        ComplexNum[] w = new ComplexNum[data.length];
    ...
    public void magnitude(ComplexNum[] data, float[] magnitude){
        for (int i = 0; i < data.length ; i++){
            magnitude[i] = (float) Math.sqrt(data[i].real * data[i].real + data[i].imag * data[i].imag);
        }
    }
    public float[] magnitude(ComplexNum[] data)
    {
    ...
    private void ChangeSeat(ComplexNum DataInput[])
    {
    ...
    //FFT运算函数
    public void FFT(ComplexNum []data)
    {
        int L=0,B=0,J=0,K=0;
        int step=0, KB=0;
        //ElemType P=0;
        ...
        ComplexNum Temp_XX = new ComplexNum();
    ...
    //IFFT运算函数
    public void IFFT(ComplexNum []data)
    {
        Log.d("Roc", "IFFT: Running");
        int L=0,B=0,J=0,K=0;
        int step=0, KB=0;
        //ElemType P=0;
        float angle=0.0f;
